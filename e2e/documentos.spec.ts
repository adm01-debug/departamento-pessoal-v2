...
test.describe("documentos E2E Tests", () => {
  ...
  test("should load documentos page", async ({ page }) => {
    await expect(page).toHaveTitle(/Departamento Pessoal/);
  });

  test("should navigate to documentos", async ({ page }) => {
    await page.click("[data-testid='nav-documentos']");
    await expect(page.locator("h1")).toBeVisible();
  });

  test("should display documentos content", async ({ page }) => {
    await page.goto("/documentos");
    await expect(page.locator("main")).toBeVisible();
  });

  test("should handle documentos interactions", async ({ page }) => {
    await page.goto("/documentos");
