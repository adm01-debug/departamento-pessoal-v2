...
test.describe("pagination E2E Tests", () => {
  ...
  test("should load pagination page", async ({ page }) => {
    await expect(page).toHaveTitle(/Departamento Pessoal/);
  });

  test("should navigate to pagination", async ({ page }) => {
    await page.click("[data-testid='nav-pagination']");
    await expect(page.locator("h1")).toBeVisible();
  });

  test("should display pagination content", async ({ page }) => {
