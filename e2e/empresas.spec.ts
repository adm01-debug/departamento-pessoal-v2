...
test.describe("empresas E2E Tests", () => {
  ...
  test("should load empresas page", async ({ page }) => {
    await expect(page).toHaveTitle(/Departamento Pessoal/);
  });

  test("should navigate to empresas", async ({ page }) => {
    await page.click("[data-testid='nav-empresas']");
    await expect(page.locator("h1")).toBeVisible();
  });

  test("should display empresas content", async ({ page }) => {
    await page.goto("/empresas");
    await expect(page.locator("main")).toBeVisible();
  });

  test("should handle empresas interactions", async ({ page }) => {
    await page.goto("/empresas");
