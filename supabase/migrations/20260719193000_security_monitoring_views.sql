...
SELECT
  ...
  COUNT(DISTINCT usuario_id)::TEXT AS value
FROM public.audit_log
WHERE acao = 'LOGIN' AND created_at > NOW() - INTERVAL '1 hour'
...
-- 4) RLS on security views (admin only)
ALTER VIEW public.v_security_dashboard SET (security_invoker = true);
ALTER VIEW public.v_security_brute_force_targets SET (security_invoker = true);
ALTER VIEW public.v_audit_summary_7d SET (security_invoker = true);
