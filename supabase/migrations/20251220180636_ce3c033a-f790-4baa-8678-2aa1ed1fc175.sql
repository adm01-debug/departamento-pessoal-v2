...
DROP POLICY IF EXISTS "Authenticated users can upload signatures" ON public.storage;
CREATE POLICY "Authenticated users can upload signatures"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'assinaturas' AND auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Authenticated users can view signatures" ON public.storage;
CREATE POLICY "Authenticated users can view signatures"
ON storage.objects FOR SELECT
USING (bucket_id = 'assinaturas' AND auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Public can upload signatures via token" ON public.storage;
CREATE POLICY "Public can upload signatures via token"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'assinaturas');

DROP POLICY IF EXISTS "Public can view signatures via token" ON public.storage;
