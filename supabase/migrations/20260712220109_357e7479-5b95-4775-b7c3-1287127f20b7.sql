...
-- View + functions wrapped in DO/EXCEPTION: multiple audit tables may be absent on preview branches
DO $$
BEGIN
  EXECUTE $q$
  ...
  $q$;
  EXECUTE 'REVOKE ALL ON public.v_audit_events_unified FROM PUBLIC, anon';
  EXECUTE 'GRANT SELECT ON public.v_audit_events_unified TO authenticated';
EXCEPTION WHEN others THEN
  NULL;
END $$;

-- Functions referencing the view — also guarded: LANGUAGE sql validates body at create time
DO $$
BEGIN
  EXECUTE $q$
    ...
    $f$
  $q$;
  EXECUTE 'REVOKE ALL ON FUNCTION public.get_audit_trail_by_user(uuid, integer) FROM PUBLIC, anon';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.get_audit_trail_by_user(uuid, integer) TO authenticated';
EXCEPTION WHEN others THEN
  NULL;
END $$;
...
BEGIN
  EXECUTE $q$
    ...
    $f$
  $q$;
  EXECUTE 'REVOKE ALL ON FUNCTION public.get_audit_trail_by_entity(text, uuid, integer) FROM PUBLIC, anon';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.get_audit_trail_by_entity(text, uuid, integer) TO authenticated';
EXCEPTION WHEN others THEN
  NULL;
END $$;
