...
CREATE TYPE public.tipo_evento_folha AS ENUM ('provento', 'desconto', 'informativo');
CREATE TYPE public.status_folha AS ENUM ('aberta', 'calculada', 'fechada', 'paga');

-- Tabela de rubricas/eventos (catálogo de eventos)
CREATE TABLE public.rubricas_folha (
  ...
  formula TEXT, -- fórmula de cálculo se automático
  ativo BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Inserir rubricas padrão
...
CREATE POLICY "Authenticated users can view rubricas" ON public.rubricas_folha FOR SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated users can manage rubricas" ON public.rubricas_folha FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Tabela de folhas de pagamento (competências)
CREATE TABLE public.folhas_pagamento (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  competencia TEXT NOT NULL, -- formato: YYYY-MM
  tipo TEXT NOT NULL DEFAULT 'mensal', -- mensal, 13_1_parcela, 13_2_parcela, ferias, rescisao
...
CREATE POLICY "Authenticated users can manage folhas" ON public.folhas_pagamento FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE TABLE public.holerites (
  ...
  -- Dados do colaborador no momento do cálculo (snapshot)
...
CREATE POLICY "Authenticated users can manage holerites" ON public.holerites FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE TABLE public.lancamentos_folha (
  ...
  tipo tipo_evento_folha NOT NULL,
  referencia DECIMAL(10,2), -- horas, dias, percentual
...
CREATE POLICY "Authenticated users can manage lancamentos" ON public.lancamentos_folha FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Tabela de eventos variáveis (lançamentos antes do cálculo)
CREATE TABLE public.eventos_variaveis (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE POLICY "Authenticated users can manage eventos_variaveis" ON public.eventos_variaveis FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Tabela de parâmetros fiscais (tabelas INSS, IRRF)
CREATE TABLE public.parametros_fiscais (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tipo TEXT NOT NULL, -- 'inss', 'irrf', 'salario_minimo', 'teto_inss'
  vigencia_inicio DATE NOT NULL,
  vigencia_fim DATE,
  faixa INTEGER, -- número da faixa para tabelas progressivas
  valor_inicial DECIMAL(12,2),
  valor_final DECIMAL(12,2),
  aliquota DECIMAL(6,4),
  deducao DECIMAL(12,2),
  valor_fixo DECIMAL(12,2), -- para salário mínimo, teto, etc.
  ativo BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE POLICY "Authenticated users can view parametros" ON public.parametros_fiscais FOR SELECT TO authenticated USING (true);