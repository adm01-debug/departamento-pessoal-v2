...
-- Empresas policies
CREATE POLICY empresa_select ON empresas FOR SELECT USING (id = auth.user_empresa_id() OR auth.jwt() ->> 'role' = 'super_admin');
CREATE POLICY empresa_insert ON empresas FOR INSERT WITH CHECK (auth.jwt() ->> 'role' = 'super_admin');
CREATE POLICY empresa_update ON empresas FOR UPDATE USING (id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'super_admin'));
CREATE POLICY empresa_delete ON empresas FOR DELETE USING (auth.jwt() ->> 'role' = 'super_admin');

-- Colaboradores policies
CREATE POLICY colab_select ON colaboradores FOR SELECT USING (empresa_id = auth.user_empresa_id());
CREATE POLICY colab_insert ON colaboradores FOR INSERT WITH CHECK (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'rh'));
CREATE POLICY colab_update ON colaboradores FOR UPDATE USING (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'rh'));
CREATE POLICY colab_delete ON colaboradores FOR DELETE USING (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' = 'admin');

-- Departamentos policies
CREATE POLICY depto_select ON departamentos FOR SELECT USING (empresa_id = auth.user_empresa_id());
CREATE POLICY depto_all ON departamentos FOR ALL USING (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'rh'));

-- Cargos policies
CREATE POLICY cargos_select ON cargos FOR SELECT USING (empresa_id = auth.user_empresa_id());
CREATE POLICY cargos_all ON cargos FOR ALL USING (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'rh'));

-- Folha policies
CREATE POLICY folha_select ON folha_pagamento FOR SELECT USING (empresa_id = auth.user_empresa_id());
CREATE POLICY folha_all ON folha_pagamento FOR ALL USING (empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' IN ('admin', 'rh', 'financeiro'));
...
CREATE POLICY audit_select ON auditoria FOR SELECT USING (
  empresa_id = auth.user_empresa_id() AND auth.jwt() ->> 'role' = 'admin'
);
CREATE POLICY audit_insert ON auditoria FOR INSERT WITH CHECK (true);

-- Notificacoes policies
CREATE POLICY notif_select ON notificacoes FOR SELECT USING (usuario_id = auth.uid() OR empresa_id = auth.user_empresa_id());
