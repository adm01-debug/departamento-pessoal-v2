...
CREATE TABLE public.afdt_importacoes (
...
CREATE TABLE public.afdt_registros_raw (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_afdt_imp_empresa ON public.afdt_importacoes(empresa_id, created_at DESC);
CREATE INDEX idx_afdt_raw_imp ON public.afdt_registros_raw(importacao_id);
CREATE INDEX idx_afdt_raw_cpf ON public.afdt_registros_raw(empresa_id, cpf) WHERE cpf IS NOT NULL;
CREATE INDEX idx_afdt_raw_data ON public.afdt_registros_raw(empresa_id, data_hora_marcacao) WHERE data_hora_marcacao IS NOT NULL;
