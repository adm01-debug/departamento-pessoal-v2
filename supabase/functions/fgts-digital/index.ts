...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!cc.ok) return createErrorResponse(cc.msg!, 422, 'INVALID_COMPETENCIA', undefined, req);

    // vencimento não pode ser retroativo
    const hoje = new Date().toISOString().slice(0, 10);
    if (p.vencimento < hoje) {
      return createErrorResponse('Vencimento retroativo', 422, 'INVALID_DUE_DATE', undefined, req);
    }
    ...
    return createErrorResponse('Erro ao gerar guia FGTS Digital', 500, 'INTERNAL_ERROR', undefined, req);
  }
});
