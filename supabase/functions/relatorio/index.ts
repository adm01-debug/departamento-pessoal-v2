...
function validateWindow(dataInicio: string, dataFim: string): Response | null {
  ...
  if (!Number.isFinite(ini) || !Number.isFinite(fim)) {
    return createErrorResponse('Datas inválidas', 422, 'INVALID_DATES', undefined, req);
  }
  if (fim < ini) {
    return createErrorResponse('dataFim < dataInicio', 422, 'INVALID_WINDOW', undefined, req);
  }
  ...
  try {
    ...
    if (filtros.tipo === 'folha') {
      const { data, error } = await admin
        ...
        .range(from, to);
      if (error) return createErrorResponse('Falha na query', 500, 'QUERY_FAILED', undefined, req);
