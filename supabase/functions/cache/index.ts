    ...
    switch (body.action) {
      ...
      case 'set': {
        ...
        if (serialized.length > MAX_VALUE_BYTES) {
          return createErrorResponse('Payload excede 256KB', 413, 'PAYLOAD_TOO_LARGE', undefined, req);
        }
      ...
      case 'query_cached': {
        if (!ALLOWED_TABLES.has(body.table)) {
          return createErrorResponse('Tabela não permitida em cache', 403, 'FORBIDDEN_TABLE', undefined, req);
        }
        ...
        if (!belongs) {
          ...
          if (!isAdmin) return createErrorResponse('Sem acesso a esta empresa', 403, 'FORBIDDEN', undefined, req);
        }
        ...
        if (error) {
          ...
          return createErrorResponse('Falha ao consultar', 500, 'QUERY_ERROR', undefined, req);
        }
