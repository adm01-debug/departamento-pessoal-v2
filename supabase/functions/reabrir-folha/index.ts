...
import { corsHeadersObj, corsHeaders, createErrorResponse, validateRequest, enforceOrigin, handlePreflight } from '../_shared/contract.ts';
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (folhaErr) return createErrorResponse('Erro ao carregar folha', 500, 'FOLHA_LOAD_ERROR', undefined, req);
    if (!folha) return createErrorResponse('Folha não encontrada', 404, 'FOLHA_NOT_FOUND', undefined, req);
    ...
    if (updErr) return createErrorResponse('Erro ao reabrir folha', 500, 'FOLHA_UPDATE_ERROR', undefined, req);
    if (!updated) {
      return createErrorResponse(
    ...
    if (auditErr) {
      ...
      return createErrorResponse('Auditoria falhou — reabertura revertida', 500, 'AUDIT_FAILED', undefined, req);
    }
