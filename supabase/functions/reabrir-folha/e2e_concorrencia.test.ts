// E2E — Concorrência sobre `reabrir-folha`
// Mesmo padrão de fechar-folha. Requer folha em status "fechada".
//
// Env: E2E_SUPABASE_URL, E2E_ANON_KEY, E2E_JWT, E2E_FOLHA_ID, E2E_MOTIVO
...
const motivo = Deno.env.get("E2E_MOTIVO") ?? "Reabertura E2E — validação de idempotência e integrity_hash.";
...
Deno.test({
  name: "reabrir-folha: N chamadas paralelas com mesma Idempotency-Key convergem",
  ignore: !canRun,
  async fn() {
    const key = `e2e-reopen-${crypto.randomUUID()}`;
    const body = JSON.stringify({ folha_id: folhaId, motivo, idempotency_key: key });

    const fire = () =>
      fetch(`${url}/functions/v1/reabrir-folha`, {
        method: "POST",
        headers: {
