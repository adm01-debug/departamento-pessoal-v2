...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (colError || !colaborador) {
      return createErrorResponse('Colaborador não encontrado', 404, 'NOT_FOUND', undefined, req);
    }
    ...
    if (!isOwner && !belongs && !isAdm) {
      return createErrorResponse('Sem acesso a este holerite', 403, 'FORBIDDEN', undefined, req);
    }
    ...
    try { captureException(error as Error, { fn: 'gerar-holerite' }); } catch { /* noop */ }
    // Erro genérico — nunca vazar mensagem/stack ao cliente (PII sensível)
    return createErrorResponse('Erro interno ao gerar holerite', 500, 'INTERNAL_SERVER_ERROR', undefined, req);
  }
});
