...
serve(async (req) => {
  ...
  try {
    ...
    if (!empresaId && !isAdm) {
      return createErrorResponse('empresaId é obrigatório', 400, 'EMPRESA_REQUIRED', undefined, req);
    }
    if (empresaId) {
    ...
    if (action === 'colaboradores') {
      ...
      if (auditErr) return createErrorResponse('Auditoria obrigatória falhou', 500, 'AUDIT_FAILED', undefined, req);
    ...
    return createErrorResponse('Ação inválida', 400, 'INVALID_ACTION', undefined, req);
  } catch (error) {
    try { captureException(error as Error, { fn: 'exportacao' }); } catch { /* noop */ }
