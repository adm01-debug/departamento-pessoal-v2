...
import { corsHeadersObj, corsHeaders, parseJsonBody } from '../_shared/contract.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) return new Response(JSON.stringify({ error: "no_auth" }), { status: 401, headers: { ...corsHeadersObj, "Content-Type": "application/json" } });
    ...
    if (!user) return new Response(JSON.stringify({ error: "invalid_auth" }), { status: 401, headers: { ...corsHeadersObj, "Content-Type": "application/json" } });

    const adminRL = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!, { auth: { persistSession: false, autoRefreshToken: false } });
    ...
    if (!parsed.success) {
      return new Response(JSON.stringify({ error: "validation", detail: parsed.error.flatten() }), { status: 400, headers: { ...corsHeadersObj, "Content-Type": "application/json" } });
    }
    ...
    if (!vinculo) return new Response(JSON.stringify({ error: "forbidden_empresa" }), { status: 403, headers: { ...corsHeadersObj, "Content-Type": "application/json" } });
    ...
    captureException(e);
    return new Response(JSON.stringify({ error: "internal" }), { status: 500, headers: { ...corsHeadersObj, "Content-Type": "application/json" } });
  }
});
