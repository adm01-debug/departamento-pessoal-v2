...
serve(async (req: Request): Promise<Response> => {
  ...
  if (!parsed.success) {
    return createErrorResponse('Parâmetros inválidos', 400, 'VALIDATION_ERROR', undefined, req);
  }
  ...
  try {
    ...
    if (mErr || !medida) return createErrorResponse('Medida não encontrada', 404, 'NOT_FOUND', undefined, req);

    // Gate multi-tenant
    const { data: belongs } = await admin.rpc('user_belongs_to_empresa', {
      _user_id: userId, _empresa_id: medida.empresa_id,
    });
    if (!belongs) return createErrorResponse('Sem acesso a esta medida', 403, 'FORBIDDEN', undefined, req);
    ...
    if (!colaborador || !empresa) {
      return createErrorResponse('Dados de contexto ausentes', 404, 'NOT_FOUND', undefined, req);
    }
    ...
    if (upErr) {
      ...
      return createErrorResponse('Falha ao armazenar documento', 500, 'STORAGE_ERROR', undefined, req);
    }
    ...
    return createErrorResponse('Erro interno ao gerar documento', 500, 'INTERNAL_SERVER_ERROR', undefined, req);
  }
});
