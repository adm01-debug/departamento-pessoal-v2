...
const ALLOWED_ORIGINS = [
  'https://frjbfeamybqsejlvmqbl.supabase.co',
  'http://localhost:8080',
  'http://localhost:3000',
  'http://localhost:5173',
  ..._extraOrigins,
...
/**
 * Default CORS headers (sem request — fallback para ALLOWED_ORIGINS[0]).
 * Use quando NÃO precisar de origem dinâmica (ex: response de erro dentro de
 * helper que não recebe a request). Para CORS dinâmico baseado no header
 * `Origin` da request, use `corsHeaders(req)`.
 */
export const corsHeadersObj: Record<string, string> = corsHeaders();
