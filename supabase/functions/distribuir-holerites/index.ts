...
serve(async (req: Request): Promise<Response> => {
  ...
  if (!folhaId) return createErrorResponse('folha_id obrigatório', 400, 'BAD_REQUEST', undefined, req);
  ...
  const { data: folha, error: folhaErr } = await admin
    ...
    .maybeSingle();
  if (folhaErr || !folha) {
    return createErrorResponse('Folha não encontrada', 404, 'NOT_FOUND', undefined, req);
  }
  ...
  const { data: vinc } = await admin
    ...
    .maybeSingle();
  if (!vinc) return createErrorResponse('Sem permissão nesta empresa', 403, 'FORBIDDEN', undefined, req);
  ...
  const { data: holerites, error: hErr } = await admin
    ...
    .eq('folha_id', folhaId);
  if (hErr) return createErrorResponse(hErr.message, 500, 'DB_ERROR', undefined, req);
  if (!holerites?.length) {
    return createErrorResponse('Nenhum holerite encontrado', 404, 'NOT_FOUND', undefined, req);
  }
  ...
  if (distErr) {
    ...
    return createErrorResponse(distErr.message, 500, 'DB_ERROR', undefined, req);
  }
