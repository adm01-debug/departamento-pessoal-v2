...
serve(async (req: Request): Promise<Response> => {
  ...
  return withMonitoring(req, 'webhook', async (supabase) => {
    ...
    if (!checkIpRate(ip)) {
      return createErrorResponse('Too many requests', 429, 'RATE_LIMITED', undefined, req);
    }
    ...
    if (payloadText.length > MAX_PAYLOAD_BYTES) {
      return createErrorResponse('Payload muito grande', 413, 'PAYLOAD_TOO_LARGE', undefined, req);
    }
    ...
    if (!timestampHeader) {
      if (REQUIRE_TIMESTAMP) {
        return createErrorResponse('Timestamp obrigatório (x-webhook-timestamp)', 401, 'TIMESTAMP_REQUIRED', undefined, req);
      }
    } else {
      const ts = Number(timestampHeader);
      if (!Number.isFinite(ts)) {
        return createErrorResponse('Timestamp inválido', 401, 'INVALID_TIMESTAMP', undefined, req);
      }
      const skewSec = Math.abs(Date.now() / 1000 - ts);
      if (skewSec > REPLAY_TTL_SECONDS) {
        return createErrorResponse('Timestamp fora da janela', 401, 'TIMESTAMP_EXPIRED', undefined, req);
      }
    }
    ...
    if (!(await verifySignature(signedPayload, signature, secret))) {
      return createErrorResponse('Assinatura inválida', 401, 'INVALID_SIGNATURE', undefined, req);
    }
    ...
    if (existing) {
      return new Response(
        ...
        { headers: { ...corsHeadersObj, 'Content-Type': 'application/json', 'Cache-Control': 'no-store' }, status: 200 }
      );
    }
    ...
    if (insertErr) {
      ...
      return createErrorResponse('Erro ao registrar webhook', 500, 'LOG_ERROR', undefined, req);
    }
