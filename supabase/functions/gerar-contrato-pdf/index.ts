...
import { corsHeadersObj, corsHeaders, createErrorResponse, parseJsonBody, enforceOrigin, handlePreflight } from '../_shared/contract.ts';
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (!parsed.success) return createErrorResponse('Parâmetros inválidos', 400, 'VALIDATION_ERROR', undefined, req);
  ...
  try {
    ...
    if (!tmplId) {
      ...
      if (error) return createErrorResponse(error.message, 403, 'FORBIDDEN', undefined, req);
      tmplId = resolved as string | null;
    }
    if (!tmplId) return createErrorResponse('Nenhum modelo de contrato aplicável — cadastre um em Configurações › Contratos.', 404, 'TEMPLATE_NOT_FOUND', undefined, req);

    const { data: varsData, error: varsErr } = await userClient.rpc('contrato_montar_variaveis', { p_admissao_id: admissao_id });
    if (varsErr) return createErrorResponse(varsErr.message, 403, 'FORBIDDEN', undefined, req);
    ...
    const { data: tmpl, error: tErr } = await admin
      ...
      .eq('id', tmplId)
      .maybeSingle();
    if (tErr || !tmpl) return createErrorResponse('Modelo não encontrado', 404, 'NOT_FOUND', undefined, req);
    ...
    if (!empresaId || empresaId !== tmpl.empresa_id) {
      return createErrorResponse('Modelo pertence a outra empresa', 403, 'FORBIDDEN', undefined, req);
    }
    ...
    if (upErr) {
      ...
      return createErrorResponse('Falha ao armazenar contrato', 500, 'STORAGE_ERROR', undefined, req);
    }
    ...
    if (insErr) {
      ...
      return createErrorResponse('Falha ao registrar contrato', 500, 'DB_ERROR', undefined, req);
    }
    ...
    return createErrorResponse('Erro interno ao gerar contrato', 500, 'INTERNAL_SERVER_ERROR', undefined, req);
  }
});
