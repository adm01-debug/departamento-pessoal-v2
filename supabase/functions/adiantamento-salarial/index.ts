...
import { corsHeadersObj, corsHeaders, createErrorResponse, createValidationErrorResponse, parseJsonBody } from '../_shared/contract.ts';
import { verifyCsrf } from '../_shared/csrf.ts';
import { captureException } from '../_shared/sentry.ts';

const noStore = { ...corsHeadersObj, 'Content-Type': 'application/json', 'Cache-Control': 'no-store' };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!pertence && !isAdmin) {
      return createErrorResponse('Acesso negado', 403, 'FORBIDDEN', undefined, req);
    }
    ...
    if (!colab) return createErrorResponse('Colaborador não encontrado', 404, 'NOT_FOUND', undefined, req);
    if (colab.status !== 'ativo') {
      return createErrorResponse('Colaborador não está ativo', 422, 'INACTIVE_EMPLOYEE', undefined, req);
    }

    const salarioCentavos = BigInt(Math.round(Number(colab.salario_base ?? 0) * 100));
    if (salarioCentavos <= 0n) {
      return createErrorResponse('Salário base do colaborador é inválido', 422, 'INVALID_SALARY', undefined, req);
    }
