...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!lote) return createErrorResponse('Lote não encontrado', 404, 'NOT_FOUND', undefined, req);
    ...
    if (aprovadoresPrevios.has(userId)) {
      return createErrorResponse('Você já aprovou este lote', 409, 'ALREADY_APPROVED', undefined, req);
    }
