  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      return new Response(JSON.stringify({ error: 'Serviço de email não configurado' }), {
        status: 503, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!empresaId || typeof empresaId !== 'string') {
      return new Response(JSON.stringify({ error: 'empresaId é obrigatório' }), {
        status: 422, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
      });
    }
    ...
    try { captureException(error as Error, { fn: 'alertas-dp' }); } catch { /* noop */ }
