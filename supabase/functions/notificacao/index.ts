...
import { corsHeadersObj, corsHeaders, createErrorResponse, validateRequest } from '../_shared/contract.ts';
    ...
    switch (body.action) {
      case 'enviar': {
        ...
        const { data: inserted, error: insErr } = await admin
          ...
          .select('id, user_id');
        if (insErr) {
          return createErrorResponse('Falha ao criar notificações', 500, 'INSERT_FAILED', undefined, req);
        }
      ...
      case 'listar': {
        ...
        if (error) {
          return createErrorResponse('Falha ao listar', 500, 'QUERY_FAILED', undefined, req);
        }
