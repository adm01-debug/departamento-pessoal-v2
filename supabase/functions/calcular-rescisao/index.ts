...
import { corsHeadersObj, corsHeaders, parseJsonBody, enforceOrigin, handlePreflight } from '../_shared/contract.ts';
...
Deno.serve(async (req) => {
  ...
  const __og = enforceOrigin(req); if (__og) return __og;try {
    ...
    try { captureException(error as Error, { fn: 'calcular-rescisao' }); } catch { /* noop */ }
