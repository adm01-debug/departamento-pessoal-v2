...
import { corsHeadersObj, corsHeaders, createErrorResponse, parseJsonBody } from '../_shared/contract.ts';
...
function json(body: unknown, status = 200): Response {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...corsHeadersObj, ...NO_STORE, 'Content-Type': 'application/json' },
  });
}
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!parsed.success) {
      return createErrorResponse('Payload inválido', 422, 'VALIDATION_ERROR', undefined, req);
    }
    ...
    try { captureException(error as Error, { fn: 'auditoria' }); } catch { /* noop */ }
    return createErrorResponse('Erro interno na auditoria', 500, 'INTERNAL_SERVER_ERROR', undefined, req);
  }
});
