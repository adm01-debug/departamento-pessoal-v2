...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (updErr) return createErrorResponse('Erro ao fechar folha', 500, 'FOLHA_UPDATE_ERROR', undefined, req);
    if (!updated) {
    ...
    if (auditErr) {
      ...
      return createErrorResponse('Auditoria falhou — fechamento revertido', 500, 'AUDIT_FAILED', undefined, req);
    }
