...
serve(async (req: Request): Promise<Response> => {
  ...
  if (req.method !== 'POST') {
    return new Response(JSON.stringify({ error: 'Method not allowed' }), {
      status: 405, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
    });
  }
  ...
  try {
    ...
    if (fotoBase64.length > 2 * 1024 * 1024) {
      return new Response(JSON.stringify({ error: 'Foto excede limite de 2MB' }), {
        status: 413, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (cError || !colaborador) {
      return new Response(JSON.stringify({ error: 'Colaborador nao encontrado' }), {
        status: 404, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!belongs) {
      return new Response(JSON.stringify({ error: 'Sem acesso a este colaborador' }), {
        status: 403, headers: { ...corsHeadersObj, 'Content-Type': 'application/json' },
      });
    }
    ...
    // Attempt real AI comparison via configured AI API
    ...
    const status = AI_API_KEY ? (isMatch ? 'valido' : 'invalido') : 'pendente';
    ...
    const { error: updError } = await supabase
      ...
      .update({
        ...
        device_metadata: {
          ...
          validado_por: AI_API_KEY ? 'AI Biometry Engine' : 'Pendente - servico indisponivel',
        }
      })
      .eq('id', batidaId)
      .eq('colaborador_id', colaboradorId);
    ...
    try { captureException(error as Error, { fn: 'validar-biometria' }); } catch { /* noop */ }
