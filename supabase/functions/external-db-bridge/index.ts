...
function jsonError(req: Request, status: number, code: string, message: string, extra?: Record<string, unknown>) {
...
function jsonOk(req: Request, payload: Record<string, unknown>) {
...
Deno.serve(async (req) => {
  ...
  const __og = enforceOrigin(req); if (__og) return __og;if (req.method !== "POST") return jsonError(req,405, "METHOD_NOT_ALLOWED", "Only POST is allowed");
  ...
  if (contentLength > MAX_PAYLOAD_BYTES) {
    return jsonError(req,413, "PAYLOAD_TOO_LARGE", `Payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  ...
  try {
        ...
        try {
          ...
          for await (const chunk of decompressedStream as unknown as AsyncIterable<Uint8Array>) {
            ...
            if (decompressedTotal > MAX_PAYLOAD_BYTES * 4) {
              ...
              return jsonError(req,413, "PAYLOAD_TOO_LARGE", "Decompressed payload too large (gzip bomb?)");
            }
          ...
          return jsonError(req,400, "INVALID_GZIP", "Failed to decompress gzip body");
        }
      } else {
    ...
    return jsonError(req,400, "INVALID_JSON", "Invalid JSON body");
  }

  const parsed = BodySchema.safeParse(rawBody);
  if (!parsed.success) {
    return jsonError(req,400, "SCHEMA_VALIDATION", "Invalid request shape", {
  ...
  if ((isWrite || isProtectedRpc) && !user) {
    return jsonError(req,401, "UNAUTHORIZED", "Authentication required for this operation");
  }
  ...
  if (action !== "rpc") {
    if (!isSafeTableName(table)) {
      return jsonError(req,400, "INVALID_TABLE", "Table name is invalid");
    }
    if (TABLE_DENYLIST.has(table!)) {
      return jsonError(req,403, "TABLE_DENIED", `Table '${table}' is not accessible via bridge`);
    }
    if (columns !== undefined && !isSafeColumnsExpr(columns)) {
      return jsonError(req,400, "INVALID_COLUMNS", "Columns expression is invalid");
    }
  }
  ...
  for (const f of filters) {
    if (!FILTER_OPS.has(f.op)) {
      return jsonError(req,400, "INVALID_OP", `Filter operator '${f.op}' is not allowed`);
    }
    if (f.op === "not" && (!f.extraOp || !NOT_EXTRA_OPS.has(f.extraOp))) {
      return jsonError(req,400, "INVALID_NOT_OP", "Filter 'not' requires a valid extraOp");
    }
    if (f.op !== "or" && !isSafeFilterColumn(f.column)) {
      return jsonError(req,400, "INVALID_COLUMN", `Filter column '${f.column}' is invalid`);
    }
  }
  ...
  if (action === "update" || action === "delete") {
    ...
    if (filters.length === 0) {
      return jsonError(req,400, "WRITE_REQUIRES_FILTER", `${action} requires at least one 'eq' filter`);
    }
  }

  // Externo — fallback automático para o próprio projeto Supabase quando
  // EXTERNAL_DB_URL/EXTERNAL_DB_KEY não estiverem setados (cenário pós-migração
  // em que o app e o banco vivem no mesmo Supabase).
  const externalUrl =
    Deno.env.get("EXTERNAL_DB_URL") || Deno.env.get("SUPABASE_URL");
  const externalKey =
    Deno.env.get("EXTERNAL_DB_KEY") || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!externalUrl || !externalKey) {
    return jsonError(req,500, "NOT_CONFIGURED", "External database not configured");
  }
  ...
  if (isWrite && user && localClient && table && TENANT_SCOPED_TABLES.has(table)) {
    ...
    if (!scope.ok) {
      return jsonError(req,403, "TENANT_SCOPE_DENIED", scope.msg);
    }
  }
  ...
  try {
    ...
    if (action === "select") {
      ...
      for (const f of filters) {
        if (f.op === "eq") query = query.eq(f.column, f.value);
          ...
          if (!isSafeOrExpression(f.value)) return jsonError(req,400, "INVALID_OR_FILTER", "Expressão .or() contém operadores ou padrões não permitidos");
      ...
      if (body.order && !isSafeOrderColumn(body.order.column)) {
        return jsonError(req,400, "INVALID_ORDER_COLUMN", "ORDER BY column contains invalid characters");
      }
      if (body.order) query = query.order(body.order.column, { ascending: body.order.ascending !== false });
      // P0-logs: usar maybeSingle() em vez de single() para não quebrar com 0 rows.
      // Vários services (.maybeSingle()) já assumem "null se vazio"; sem isso, qualquer
      // select com single=true numa tabela vazia virava 400 + BRIDGE_SCHEMA_ERROR.
      ...
      if (error) { console.error('[bridge] QUERY_ERROR:', error.message, error.hint); return jsonError(req,400, "QUERY_ERROR", `Falha na consulta: ${error.message} (${error.code || ''})`); }
      return jsonOk(req,{ data: selectData, count, duration_ms: durationMs });
    }
    ...
    if (action === "insert") {
      ...
      if (error) { console.error('[bridge] INSERT_ERROR:', error.message, error.hint); return jsonError(req,400, "INSERT_ERROR", "Falha na inserção"); }
      return jsonOk(req,{ data: r, duration_ms: durationMs });
    }
    ...
    if (action === "upsert") {
      ...
      if (error) { console.error('[bridge] UPSERT_ERROR:', error.message, error.hint); return jsonError(req,400, "UPSERT_ERROR", "Falha no upsert"); }
      return jsonOk(req,{ data: r, duration_ms: durationMs });
    }
    ...
    if (action === "update") {
      if (filters.length === 0) {
        return jsonError(req,400, "UPDATE_REQUIRES_FILTER", "UPDATE requires at least one filter");
      }
      if (!filters.some((f) => f.op === "eq")) {
        return jsonError(req,400, "UPDATE_REQUIRES_EQ", "UPDATE requires at least one 'eq' filter for safety");
      }
      const t0 = performance.now();
      ...
      if (error) { console.error('[bridge] UPDATE_ERROR:', error.message, error.hint); return jsonError(req,400, "UPDATE_ERROR", "Falha na atualização"); }
      return jsonOk(req,{ data: r, duration_ms: durationMs });
    }
    ...
    if (action === "delete") {
      if (filters.length === 0) {
        return jsonError(req,400, "DELETE_REQUIRES_FILTER", "DELETE requires at least one filter");
      }
      if (!filters.some((f) => f.op === "eq")) {
        return jsonError(req,400, "DELETE_REQUIRES_EQ", "DELETE requires at least one 'eq' filter for safety");
      }
      const t0 = performance.now();
      ...
      if (error) { console.error('[bridge] DELETE_ERROR:', error.message, error.hint); return jsonError(req,400, "DELETE_ERROR", "Falha na exclusão"); }
      return jsonOk(req,{ data: r, duration_ms: durationMs });
    }
    ...
    if (action === "rpc") {
      if (!rpcName || !isSafeTableName(rpcName)) {
        return jsonError(req,400, "INVALID_RPC", "rpc action requires a valid 'rpcName' or 'fn'");
      }
      if (!RPC_ALLOWLIST.has(rpcName)) {
        return jsonError(req,403, "RPC_DENIED", `RPC '${rpcName}' is not in allowlist`);
      }
      const t0 = performance.now();
      if (rpcName in LOGIN_PROTECTION_RPC_FALLBACKS) {
        const durationMs = Math.round(performance.now() - t0);
        return jsonOk(req,{ data: LOGIN_PROTECTION_RPC_FALLBACKS[rpcName], duration_ms: durationMs });
      }
      ...
      if (error) {
        ...
        return jsonError(req,400, "RPC_ERROR", "Falha na chamada RPC");
      }
      return jsonOk(req,{ data: rpcData, duration_ms: durationMs });
    }

    return jsonError(req,400, "UNKNOWN_ACTION", `Unknown action: ${action}`);
  } catch (err) {
    const msg = err instanceof Error ? err.message : "Internal server error";
    if (isTimeoutError(err)) {
      console.error("[external-db-bridge] Timeout:", msg);
      return jsonError(req,504, "QUERY_TIMEOUT", "A consulta excedeu o tempo limite");
    }
