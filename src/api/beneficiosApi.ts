...
const TABLE = "beneficios_colaborador";

export async function list(filters?: Record<string, any>) {
  let q = (supabase.from(TABLE) as any).select("*, tipos_beneficio(*)");
  if (filters) {
...
export async function getById(id: string) {
  const { data, error } = await (supabase.from(TABLE) as any).select("*, tipos_beneficio(*)").eq("id", id).single();
  if (error) throw error;
  return data;
}
