...
export function AnalyticsSection({ stats, pendencias, isLoadingStats, isLoadingPendencias, isEmptySystem, empresaId }: AnalyticsSectionProps) {
  ...
  useEffect(() => {
    ...
    // P3-fix: usar nome único por mount. Sob React 19 StrictMode o effect roda
    // 2x e o supabase-js 2.x rejeita .on('postgres_changes', ...) depois de
    // .subscribe() no mesmo canal — o cleanup removeChannel é assíncrono e o
    // canal anterior pode ainda existir quando o novo effect cria um homônimo.
    const channelName = `notif-toast-${crypto.randomUUID()}`;
  ...
  useEffect(() => {
    if (!empresaId) return;
    const pontoChannelName = `ponto-changes-${crypto.randomUUID()}`;
    const pontoChannel = (supabase as any)
      ...
      .subscribe();
    return () => { (supabase as any).removeChannel(pontoChannel); };
  }, [empresaId]);
