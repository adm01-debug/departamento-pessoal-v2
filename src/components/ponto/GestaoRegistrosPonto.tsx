  ...
  const { data: registros = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await (supabase as any)
        ...
        .select('*, colaborador:colaboradores!fk_registros_ponto_colaborador(nome_completo, cargo, departamento, foto_url)')
  ...
  const { data: batidas = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('batidas_ponto')
        .select('*, colaborador:colaboradores!batidas_ponto_colaborador_id_fkey(nome_completo)')
        .eq('empresa_id', empresaAtual.id)
        .gte('data', filtroData)
        .lte('data', filtroFim);
      if (error) throw error;
