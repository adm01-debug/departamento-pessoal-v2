...
interface FolhaPagamentoMensalProps { className?: string; data?: any[]; onExport?: (format: string) => void; onRefresh?: () => void; loading?: boolean; }

export function FolhaPagamentoMensal({ className, data = [], onExport, onRefresh, loading = false }: FolhaPagamentoMensalProps) {
    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />FolhaPagamentoMensal</CardTitle><CardDescription>Relatório</CardDescription></div>
...
export default FolhaPagamentoMensal;
