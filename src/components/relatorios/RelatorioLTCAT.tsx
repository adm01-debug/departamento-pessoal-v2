...
export const RelatorioLTCAT: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando LTCAT - Laudo Técnico Cond. Ambientais em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />LTCAT - Laudo Técnico Cond. Ambientais</CardTitle>
        <CardDescription>Condições ambientais de trabalho</CardDescription>
      </CardHeader>
