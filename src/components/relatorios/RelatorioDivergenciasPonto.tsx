...
export const RelatorioDivergenciasPonto: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando Divergências de Ponto em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Divergências de Ponto</CardTitle>
        <CardDescription>Inconsistências nas marcações</CardDescription>
      </CardHeader>
...
export default RelatorioDivergenciasPonto;
