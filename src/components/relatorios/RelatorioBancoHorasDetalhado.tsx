...
export const RelatorioBancoHorasDetalhado: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando Banco de Horas Detalhado em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Banco de Horas Detalhado</CardTitle>
        <CardDescription>Saldo e movimentações de banco de horas</CardDescription>
      </CardHeader>
...
export default RelatorioBancoHorasDetalhado;
