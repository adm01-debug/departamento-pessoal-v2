...
export const RelatorioConectividadeSocial: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando Conectividade Social em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Conectividade Social</CardTitle>
        <CardDescription>Comunicação com a Caixa</CardDescription>
      </CardHeader>
...
export default RelatorioConectividadeSocial;
