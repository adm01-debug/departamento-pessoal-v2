...
export const RelatorioPPP: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando PPP - Perfil Profissiográfico Previdenciário em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />PPP - Perfil Profissiográfico Previdenciário</CardTitle>
        <CardDescription>Histórico laboral do trabalhador</CardDescription>
      </CardHeader>
