...
export const RelatorioEvolucaoHeadcount: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando Evolução do Headcount em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Evolução do Headcount</CardTitle>
        <CardDescription>Crescimento do quadro de pessoal</CardDescription>
      </CardHeader>
...
export default RelatorioEvolucaoHeadcount;
