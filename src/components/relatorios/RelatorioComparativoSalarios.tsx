...
export const RelatorioComparativoSalarios: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando Comparativo de Salários em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Comparativo de Salários</CardTitle>
        <CardDescription>Análise salarial por cargo e departamento</CardDescription>
      </CardHeader>
...
export default RelatorioComparativoSalarios;
