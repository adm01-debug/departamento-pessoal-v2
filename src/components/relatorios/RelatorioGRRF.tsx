...
export const RelatorioGRRF: React.FC = () => {
  ...
  const gerarRelatorio = async (formato: 'pdf' | 'excel') => {
    ...
    setLoading(false);
    console.log('Gerando GRRF - Guia de Recolhimento Rescisório do FGTS em', formato);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />GRRF - Guia de Recolhimento Rescisório do FGTS</CardTitle>
        <CardDescription>FGTS rescisório com multa 40%</CardDescription>
      </CardHeader>
