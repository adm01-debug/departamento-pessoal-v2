...
export const PGREditor: React.FC = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" />PGR - Programa de Gerenciamento de Riscos<Badge variant={status === 'vigente' ? 'default' : status === 'vencido' ? 'destructive' : 'outline'}>{status}</Badge></CardTitle>
        <CardDescription>Editor completo do PGR conforme NR-01</CardDescription>
      </CardHeader>
