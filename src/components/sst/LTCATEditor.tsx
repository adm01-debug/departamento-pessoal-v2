...
export const LTCATEditor: React.FC = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" />LTCAT - Laudo Técnico<Badge variant={status === 'vigente' ? 'default' : status === 'vencido' ? 'destructive' : 'outline'}>{status}</Badge></CardTitle>
        <CardDescription>Laudo Técnico das Condições Ambientais</CardDescription>
      </CardHeader>
