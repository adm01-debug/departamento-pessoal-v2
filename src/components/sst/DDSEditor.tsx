...
export const DDSEditor: React.FC = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" />DDS - Diálogo Diário de Segurança<Badge variant={status === 'vigente' ? 'default' : status === 'vencido' ? 'destructive' : 'outline'}>{status}</Badge></CardTitle>
        <CardDescription>Registro de diálogos de segurança</CardDescription>
      </CardHeader>
