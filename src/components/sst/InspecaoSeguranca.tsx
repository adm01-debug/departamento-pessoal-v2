...
export const InspecaoSeguranca: React.FC = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" />Inspeção de Segurança<Badge variant={status === 'vigente' ? 'default' : status === 'vencido' ? 'destructive' : 'outline'}>{status}</Badge></CardTitle>
        <CardDescription>Checklists e inspeções periódicas</CardDescription>
      </CardHeader>
...
export default InspecaoSeguranca;
