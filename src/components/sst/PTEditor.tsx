...
export const PTEditor: React.FC = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" />PT - Permissão de Trabalho<Badge variant={status === 'vigente' ? 'default' : status === 'vencido' ? 'destructive' : 'outline'}>{status}</Badge></CardTitle>
        <CardDescription>Autorização para trabalhos especiais</CardDescription>
      </CardHeader>
