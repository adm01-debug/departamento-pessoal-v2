...
import { Settings, Key, Link, Save, TestTube } from 'lucide-react';
...
export const Bitrix24ConfigForm = memo(function Bitrix24ConfigForm({ config, onSave, onTest }: Bitrix24ConfigFormProps) {
  ...
  const handleTest = async () => {
    setTesting(true);
    await onTest(formData.webhookUrl);
    setTesting(false);
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Input value={formData.webhookUrl} onChange={e => setFormData({...formData, webhookUrl: e.target.value})} placeholder="https://seudominio.bitrix24.com.br/rest/..." />
        </div>
        <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleTest} disabled={!formData.webhookUrl || testing}><TestTube className="h-4 w-4 mr-2" />{testing ? 'Testando...' : 'Testar'}</Button>
          <Button onClick={() => onSave(formData)}><Save className="h-4 w-4 mr-2" />Salvar</Button>
        </div>
      </CardContent>
    </Card>
  );
