...
interface EventoS2230Props {
  data?: EventoS2230Data;
  onSave?: (data: EventoS2230Data) => void;
  onValidate?: (data: EventoS2230Data) => Promise<boolean>;
  onSend?: (data: EventoS2230Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS2230({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS2230Props) {
  const [data, setData] = useState<EventoS2230Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S2230 - Afastamento Temporário
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S2230.</AlertDescription></Alert>
          </TabsContent>
