...
interface EventoS2200Props {
  data?: EventoS2200Data;
  onSave?: (data: EventoS2200Data) => void;
  onValidate?: (data: EventoS2200Data) => Promise<boolean>;
  onSend?: (data: EventoS2200Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS2200({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS2200Props) {
  const [data, setData] = useState<EventoS2200Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S2200 - Cadastramento Inicial
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S2200.</AlertDescription></Alert>
          </TabsContent>
