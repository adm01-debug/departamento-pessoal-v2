...
interface EventoS1010Props {
  data?: EventoS1010Data;
  onSave?: (data: EventoS1010Data) => void;
  onValidate?: (data: EventoS1010Data) => Promise<boolean>;
  onSend?: (data: EventoS1010Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS1010({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS1010Props) {
  const [data, setData] = useState<EventoS1010Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S1010 - Tabela de Rubricas
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S1010.</AlertDescription></Alert>
          </TabsContent>
