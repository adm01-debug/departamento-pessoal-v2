...
interface EventoS1030Props {
  data?: EventoS1030Data;
  onSave?: (data: EventoS1030Data) => void;
  onValidate?: (data: EventoS1030Data) => Promise<boolean>;
  onSend?: (data: EventoS1030Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS1030({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS1030Props) {
  const [data, setData] = useState<EventoS1030Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S1030 - Tabela de Cargos
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S1030.</AlertDescription></Alert>
          </TabsContent>
