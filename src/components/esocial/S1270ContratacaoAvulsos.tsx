...
export const S1270ContratacaoAvulsos: React.FC = () => {
  const [perApur, setPerApur] = useState('');
  const [status, setStatus] = useState<'rascunho' | 'validado' | 'enviado'>('rascunho');

  return (
    <Card>
      <CardHeader><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />S-1270 - Contratação de Trabalhadores Avulsos<Badge variant="outline">{status}</Badge></CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <div><Label>Período Apuração</Label><Input type="month" value={perApur} onChange={(e) => setPerApur(e.target.value)} /></div>
        <p className="text-sm text-muted-foreground">Componente para registro de informações do evento S-1270 - Contratação de Trabalhadores Avulsos</p>
