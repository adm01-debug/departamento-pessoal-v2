...
interface EventoS1005Props {
  data?: EventoS1005Data;
  onSave?: (data: EventoS1005Data) => void;
  onValidate?: (data: EventoS1005Data) => Promise<boolean>;
  onSend?: (data: EventoS1005Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS1005({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS1005Props) {
  const [data, setData] = useState<EventoS1005Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S1005 - Tabela de Estabelecimentos
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S1005.</AlertDescription></Alert>
          </TabsContent>
