...
interface EventoS1020Props {
  data?: EventoS1020Data;
  onSave?: (data: EventoS1020Data) => void;
  onValidate?: (data: EventoS1020Data) => Promise<boolean>;
  onSend?: (data: EventoS1020Data) => Promise<void>;
  loading?: boolean;
  className?: string;
}

export function EventoS1020({ data: initialData, onSave, onValidate, onSend, loading = false, className }: EventoS1020Props) {
  const [data, setData] = useState<EventoS1020Data>(initialData || { status: "rascunho" });
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" />S1020 - Tabela de Lotações
            </CardTitle>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dados" className="space-y-4 mt-4">
            ...
            <Alert><AlertDescription>Preencha todos os campos obrigatórios do evento S1020.</AlertDescription></Alert>
          </TabsContent>
