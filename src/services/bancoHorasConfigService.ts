...
export const bancoHorasConfigService = {
  async buscar(empresaId: string) {
    const { data, error } = await supabase.from('banco_horas_config').select('*').eq('empresa_id', empresaId).maybeSingle();
    if (error) throw error;
    return data;
  },
  async salvar(d: any) {
    // Upsert by empresa_id
    const existing = d.empresa_id ? await bancoHorasConfigService.buscar(d.empresa_id) : null;
    if (existing) {
      const { data, error } = await supabase.from('banco_horas_config').update(d).eq('id', existing.id).select().maybeSingle();
      if (error) throw error;
