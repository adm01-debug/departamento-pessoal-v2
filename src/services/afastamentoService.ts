...
class AfastamentoService extends BaseService<any> {
  ...
  async listar(options: ListOptions = {}): Promise<ListResponse<any>> {
    ...
    const selectStr = `
      *,
      colaborador:colaboradores!afastamentos_colaborador_id_fkey(nome_completo, departamento)
    `;
  ...
  async listarProrrogacoes(afastamentoId?: string, empresaId?: string): Promise<any[]> {
    ...
    let query = (supabase as any)
      ...
      .select('*, afastamento:afastamentos!inner(*, colaborador:colaboradores!afastamentos_colaborador_id_fkey(nome_completo))');
