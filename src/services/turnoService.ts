...
export const turnoService = {
  ...
  async listarEscalas(empresaId: string, data?: string): Promise<any[]> {
    ...
    let q = supabase.from('escalas_trabalho').select('*, colaborador:colaboradores!escalas_trabalho_colaborador_id_fkey(nome_completo), turno:turnos(nome, horario_inicio, horario_fim, cor)').order('data');
    q = q.eq('empresa_id', empresaId);
