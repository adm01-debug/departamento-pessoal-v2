...
export const catalogoCursoService = {
  ...
  async listarInscricoes(empresaId: string, cursoId?: string) {
    ...
    let q = supabase
      ...
      .select('*, colaborador:colaboradores!inscricoes_cursos_colaborador_id_fkey(nome_completo), curso:catalogo_cursos!inscricoes_cursos_curso_id_fkey(nome)')
      .eq('empresa_id', empresaId)
  ...
  async listarCertificados(empresaId: string, colaboradorId?: string) {
    ...
    let q = (supabase as any)
      ...
      .select('*, curso:catalogo_cursos!treinamento_certificados_curso_id_fkey(nome, carga_horaria), colaborador:colaboradores!treinamento_certificados_colaborador_id_fkey(nome_completo)')
      .eq('empresa_id', empresaId);
  ...
  async getCertificado(id: string) {
    const { data, error } = await (supabase as any)
      .from('treinamento_certificados')
      .select('*, curso:catalogo_cursos!treinamento_certificados_curso_id_fkey(*), colaborador:colaboradores!treinamento_certificados_colaborador_id_fkey(*)')
      .eq('id', id)
      .single();
    if (error) throw error;
    return data;
