...
export const contabilidadeService = {
  async listLancamentos(empresaId: string) {
  ...
  async listPlanoContas(empresaId: string) {
    const { data, error } = await supabase
      .from('plano_contas')
      .select('*')
      .eq('empresa_id', empresaId)
  ...
  async gerarLancamentosFolha(empresaId: string, folhaId: string) {
    // Fetch folha summary
  ...
  async exportarSPED(empresaId: string) {
    const lancamentos = await this.listLancamentos(empresaId);
    
    // Simulate SPED Layout (Text based)
