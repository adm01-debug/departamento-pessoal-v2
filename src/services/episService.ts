...
export const episService = {
  async listar(empresaId?: string): Promise<any[]> {
    ...
    let q = (supabase as Record<string, unknown>).from('epis').select('*').order('nome');
  ...
  async excluir(id: string): Promise<void> {
    ...
    const { error } = await (supabase as Record<string, unknown>).from('epis').delete().eq('id', id);
    if (error) throw error;
...
export const episEntregasService = {
  async listar(empresaId?: string): Promise<any[]> {
    ...
    let q = (supabase as Record<string, unknown>).from('epis_entregas').select('*, epi:epis(nome, ca), colaborador:colaboradores(nome_completo)').order('data_entrega', { ascending: false });
    if (empresaId) q = q.eq('empresa_id', empresaId);
    const { data, error } = await q;
    if (error) throw error;
    return data || [];
  ...
  async buscarPorColaborador(colaboradorId: string): Promise<any[]> {
    ...
    const { data, error } = await (supabase as Record<string, unknown>).from('epis_entregas').select('*, epi:epis(nome, ca)').eq('colaborador_id', colaboradorId).order('data_entrega', { ascending: false });
    if (error) throw error;
    return data || [];
  ...
  async excluir(id: string): Promise<void> {
    ...
    const { error } = await (supabase as Record<string, unknown>).from('epis_entregas').delete().eq('id', id);
    if (error) throw error;
