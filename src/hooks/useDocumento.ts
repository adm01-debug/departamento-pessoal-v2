...
interface useDocumentoData { id?: string; [key: string]: any; }
interface useDocumentoState { data: useDocumentoData[]; loading: boolean; error: string | null; }

export function useDocumento() {
  const [state, setState] = useState<useDocumentoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDocumentoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDocumentoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
