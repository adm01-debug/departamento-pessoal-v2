...
interface useDemissaoData { id?: string; [key: string]: any; }
interface useDemissaoState { data: useDemissaoData[]; loading: boolean; error: string | null; }

export function useDemissao() {
  const [state, setState] = useState<useDemissaoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDemissaoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDemissaoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
