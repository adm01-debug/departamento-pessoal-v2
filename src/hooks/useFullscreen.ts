...
interface useFullscreenData { id?: string; [key: string]: any; }
interface useFullscreenState { data: useFullscreenData[]; loading: boolean; error: string | null; }

export function useFullscreen() {
  const [state, setState] = useState<useFullscreenState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useFullscreenData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useFullscreenData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
