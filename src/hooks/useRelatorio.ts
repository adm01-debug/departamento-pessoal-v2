...
interface useRelatorioData { id?: string; [key: string]: any; }
interface useRelatorioState { data: useRelatorioData[]; loading: boolean; error: string | null; }

export function useRelatorio() {
  const [state, setState] = useState<useRelatorioState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useRelatorioData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useRelatorioData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
