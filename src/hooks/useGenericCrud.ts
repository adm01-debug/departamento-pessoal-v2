...
// Filtro "significativo" = tem algum valor além de empresa_id, status/depto/cargo
// "all" (placeholders da UI). Usado para decidir se a query do useGenericCrud
// pode rodar antes do `empresaAtual` ser resolvido — se só tem placeholders,
// dispararia o erro "empresa_id obrigatório" e polui o console com 1 query
// falha que logo é reexecutada com os filtros certos.
function hasMeaningfulFilters(filters: Record<string, unknown>): boolean {
  for (const [k, v] of Object.entries(filters)) {
    if (v === undefined || v === null) continue;
    if (k === 'empresaId' || k === 'empresa_id') continue;
    if (v === 'all' || v === '') continue;
    return true;
  }
  return false;
...
export function useGenericCrud<T>({
  ...
  const query = useQuery({
    ...
    enabled: alwaysEnabled || hasMeaningfulFilters(filters) || !!empresaId,
    queryFn: async () => {
      try {
