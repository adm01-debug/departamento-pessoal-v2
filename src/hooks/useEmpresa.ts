...
interface useEmpresaData { id?: string; [key: string]: any; }
interface useEmpresaState { data: useEmpresaData[]; loading: boolean; error: string | null; }

export function useEmpresa() {
  const [state, setState] = useState<useEmpresaState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useEmpresaData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useEmpresaData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
