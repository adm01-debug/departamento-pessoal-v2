...
interface useCargoData { id?: string; [key: string]: any; }
interface useCargoState { data: useCargoData[]; loading: boolean; error: string | null; }

export function useCargo() {
  const [state, setState] = useState<useCargoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useCargoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useCargoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
