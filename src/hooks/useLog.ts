...
interface useLogData { id?: string; [key: string]: any; }
interface useLogState { data: useLogData[]; loading: boolean; error: string | null; }

export function useLog() {
  const [state, setState] = useState<useLogState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useLogData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useLogData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
