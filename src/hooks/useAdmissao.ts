...
interface useAdmissaoData { id?: string; [key: string]: any; }
interface useAdmissaoState { data: useAdmissaoData[]; loading: boolean; error: string | null; }

export function useAdmissao() {
  const [state, setState] = useState<useAdmissaoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useAdmissaoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useAdmissaoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
