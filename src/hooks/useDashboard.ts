...
interface useDashboardData { id?: string; [key: string]: any; }
interface useDashboardState { data: useDashboardData[]; loading: boolean; error: string | null; }

export function useDashboard() {
  const [state, setState] = useState<useDashboardState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDashboardData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDashboardData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
