...
interface useNotificationData { id?: string; [key: string]: any; }
interface useNotificationState { data: useNotificationData[]; loading: boolean; error: string | null; }

export function useNotification() {
  const [state, setState] = useState<useNotificationState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useNotificationData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useNotificationData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
