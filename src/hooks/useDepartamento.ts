...
interface useDepartamentoData { id?: string; [key: string]: any; }
interface useDepartamentoState { data: useDepartamentoData[]; loading: boolean; error: string | null; }

export function useDepartamento() {
  const [state, setState] = useState<useDepartamentoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDepartamentoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDepartamentoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
