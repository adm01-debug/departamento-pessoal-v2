...
interface useImportData { id?: string; [key: string]: any; }
interface useImportState { data: useImportData[]; loading: boolean; error: string | null; }

export function useImport() {
  const [state, setState] = useState<useImportState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useImportData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useImportData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
