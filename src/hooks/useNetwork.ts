...
interface useNetworkData { id?: string; [key: string]: any; }
interface useNetworkState { data: useNetworkData[]; loading: boolean; error: string | null; }

export function useNetwork() {
  const [state, setState] = useState<useNetworkState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useNetworkData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useNetworkData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
