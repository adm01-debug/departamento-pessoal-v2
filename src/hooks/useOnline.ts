...
interface useOnlineData { id?: string; [key: string]: any; }
interface useOnlineState { data: useOnlineData[]; loading: boolean; error: string | null; }

export function useOnline() {
  const [state, setState] = useState<useOnlineState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useOnlineData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useOnlineData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
