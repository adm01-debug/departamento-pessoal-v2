...
interface useRoleData { id?: string; [key: string]: any; }
interface useRoleState { data: useRoleData[]; loading: boolean; error: string | null; }

export function useRole() {
  const [state, setState] = useState<useRoleState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useRoleData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useRoleData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
