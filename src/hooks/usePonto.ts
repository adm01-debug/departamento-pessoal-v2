...
interface usePontoData { id?: string; [key: string]: any; }
interface usePontoState { data: usePontoData[]; loading: boolean; error: string | null; }

export function usePonto() {
  const [state, setState] = useState<usePontoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<usePontoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<usePontoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
