...
interface useVisibilityData { id?: string; [key: string]: any; }
interface useVisibilityState { data: useVisibilityData[]; loading: boolean; error: string | null; }

export function useVisibility() {
  const [state, setState] = useState<useVisibilityState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useVisibilityData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useVisibilityData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
