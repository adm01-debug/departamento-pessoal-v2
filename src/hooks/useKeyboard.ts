...
interface useKeyboardData { id?: string; [key: string]: any; }
interface useKeyboardState { data: useKeyboardData[]; loading: boolean; error: string | null; }

export function useKeyboard() {
  const [state, setState] = useState<useKeyboardState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useKeyboardData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useKeyboardData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
