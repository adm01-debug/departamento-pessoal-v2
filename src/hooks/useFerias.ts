...
interface useFeriasData { id?: string; [key: string]: any; }
interface useFeriasState { data: useFeriasData[]; loading: boolean; error: string | null; }

export function useFerias() {
  const [state, setState] = useState<useFeriasState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useFeriasData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useFeriasData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
