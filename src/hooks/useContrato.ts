...
interface useContratoData { id?: string; [key: string]: any; }
interface useContratoState { data: useContratoData[]; loading: boolean; error: string | null; }

export function useContrato() {
  const [state, setState] = useState<useContratoState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useContratoData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useContratoData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
