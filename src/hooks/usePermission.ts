...
interface usePermissionData { id?: string; [key: string]: any; }
interface usePermissionState { data: usePermissionData[]; loading: boolean; error: string | null; }

export function usePermission() {
  const [state, setState] = useState<usePermissionState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<usePermissionData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<usePermissionData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
