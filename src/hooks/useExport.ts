...
interface useExportData { id?: string; [key: string]: any; }
interface useExportState { data: useExportData[]; loading: boolean; error: string | null; }

export function useExport() {
  const [state, setState] = useState<useExportState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useExportData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useExportData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
