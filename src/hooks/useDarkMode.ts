...
interface useDarkModeData { id?: string; [key: string]: any; }
interface useDarkModeState { data: useDarkModeData[]; loading: boolean; error: string | null; }

export function useDarkMode() {
  const [state, setState] = useState<useDarkModeState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDarkModeData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDarkModeData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
