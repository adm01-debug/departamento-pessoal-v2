...
interface useDependenteData { id?: string; [key: string]: any; }
interface useDependenteState { data: useDependenteData[]; loading: boolean; error: string | null; }

export function useDependente() {
  const [state, setState] = useState<useDependenteState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useDependenteData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useDependenteData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
