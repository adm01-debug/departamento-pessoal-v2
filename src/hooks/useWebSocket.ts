...
interface useWebSocketData { id?: string; [key: string]: any; }
interface useWebSocketState { data: useWebSocketData[]; loading: boolean; error: string | null; }

export function useWebSocket() {
  const [state, setState] = useState<useWebSocketState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useWebSocketData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useWebSocketData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
