...
interface useBeneficioData { id?: string; [key: string]: any; }
interface useBeneficioState { data: useBeneficioData[]; loading: boolean; error: string | null; }

export function useBeneficio() {
  const [state, setState] = useState<useBeneficioState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useBeneficioData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useBeneficioData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
