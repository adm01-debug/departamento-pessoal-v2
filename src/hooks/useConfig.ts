...
interface useConfigData { id?: string; [key: string]: any; }
interface useConfigState { data: useConfigData[]; loading: boolean; error: string | null; }

export function useConfig() {
  const [state, setState] = useState<useConfigState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useConfigData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useConfigData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
