...
interface useCacheData { id?: string; [key: string]: any; }
interface useCacheState { data: useCacheData[]; loading: boolean; error: string | null; }

export function useCache() {
  const [state, setState] = useState<useCacheState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useCacheData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useCacheData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
