...
interface useESocialData { id?: string; [key: string]: any; }
interface useESocialState { data: useESocialData[]; loading: boolean; error: string | null; }

export function useESocial() {
  const [state, setState] = useState<useESocialState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useESocialData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useESocialData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
