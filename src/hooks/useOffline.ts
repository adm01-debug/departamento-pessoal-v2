...
interface useOfflineData { id?: string; [key: string]: any; }
interface useOfflineState { data: useOfflineData[]; loading: boolean; error: string | null; }

export function useOffline() {
  const [state, setState] = useState<useOfflineState>({ data: [], loading: false, error: null });
  ...
  const create = useCallback(async (data: Omit<useOfflineData, "id">) => {
  ...
  const update = useCallback(async (id: string, data: Partial<useOfflineData>) => {
    setState(prev => ({ ...prev, loading: true }));
    try {
