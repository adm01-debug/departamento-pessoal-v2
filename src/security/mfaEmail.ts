...
export class mfaEmailSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating mfaEmail:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing mfaEmail for:', action);
...
export const createmfaEmailSecurity = (config?: SecurityConfig) => new mfaEmailSecurity(config);
export default mfaEmailSecurity;
