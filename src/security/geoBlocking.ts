...
export class geoBlockingSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating geoBlocking:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing geoBlocking for:', action);
...
export const creategeoBlockingSecurity = (config?: SecurityConfig) => new geoBlockingSecurity(config);
export default geoBlockingSecurity;
