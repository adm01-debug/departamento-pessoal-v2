...
export class ssoOpenIDConnectSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating ssoOpenIDConnect:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing ssoOpenIDConnect for:', action);
...
export const createssoOpenIDConnectSecurity = (config?: SecurityConfig) => new ssoOpenIDConnectSecurity(config);
export default ssoOpenIDConnectSecurity;
