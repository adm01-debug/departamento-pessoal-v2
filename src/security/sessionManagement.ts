...
export class sessionManagementSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating sessionManagement:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing sessionManagement for:', action);
...
export const createsessionManagementSecurity = (config?: SecurityConfig) => new sessionManagementSecurity(config);
export default sessionManagementSecurity;
