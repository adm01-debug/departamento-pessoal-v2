...
export class mfaTOTPSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating mfaTOTP:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing mfaTOTP for:', action);
...
export const createmfaTOTPSecurity = (config?: SecurityConfig) => new mfaTOTPSecurity(config);
export default mfaTOTPSecurity;
