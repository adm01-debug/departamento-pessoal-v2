...
export class biometricAuthSecurity {
  ...
  async validate(data: any): Promise<SecurityResult> {
    ...
    console.log('Validating biometricAuth:', data);
  ...
  async enforce(action: string, context: any): Promise<SecurityResult> {
    console.log('Enforcing biometricAuth for:', action);
...
export const createbiometricAuthSecurity = (config?: SecurityConfig) => new biometricAuthSecurity(config);
export default biometricAuthSecurity;
