...
import {
  ...
  Send, Eye, Download, XCircle, Users
} from 'lucide-react';
...
const MOCK_DOCS: DocumentoAssinatura[] = [
  { id: '1', titulo: 'Contrato de Trabalho - CLT', tipo: 'Contrato', colaborador: 'Maria Silva', status: 'pendente', criadoEm: '2026-03-10' },
  { id: '2', titulo: 'Termo de Confidencialidade', tipo: 'Termo', colaborador: 'João Santos', status: 'assinado', criadoEm: '2026-03-08', assinadoEm: '2026-03-09' },
  { id: '3', titulo: 'Aditivo Contratual - Promoção', tipo: 'Aditivo', colaborador: 'Ana Costa', status: 'assinado', criadoEm: '2026-03-05', assinadoEm: '2026-03-06' },
  { id: '4', titulo: 'Acordo de Banco de Horas', tipo: 'Acordo', colaborador: 'Carlos Lima', status: 'expirado', criadoEm: '2026-02-20' },
  { id: '5', titulo: 'Termo de Rescisão', tipo: 'Rescisão', colaborador: 'Pedro Alves', status: 'recusado', criadoEm: '2026-03-12' },
];
...
export default function AssinaturasPage(): React.ReactElement {
  ...
  const filtered = MOCK_DOCS
    ...
    .filter(d => d.titulo.toLowerCase().includes(search.toLowerCase()) || d.colaborador.toLowerCase().includes(search.toLowerCase()));

  const stats = {
    total: MOCK_DOCS.length,
    pendentes: MOCK_DOCS.filter(d => d.status === 'pendente').length,
    assinados: MOCK_DOCS.filter(d => d.status === 'assinado').length,
    expirados: MOCK_DOCS.filter(d => d.status === 'expirado').length,
  };
