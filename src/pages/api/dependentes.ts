...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (method) {
      ...
      case "POST": {
        const { data, error } = await supabase.from("dependentes").insert(body).select().single();
        if (error) throw error;
        return res.status(201).json({ success: true, data });
      }
      case "PUT": {
        const { id } = query;
        const { data, error } = await supabase.from("dependentes").update(body).eq("id", id).select().single();
        if (error) throw error;
        return res.status(200).json({ success: true, data });
      }
      case "DELETE": {
