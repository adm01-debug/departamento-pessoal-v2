...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (method) {
      case "GET": {
        const { data, error } = await supabase.from("backup").select("*");
        if (error) throw error;
        return res.status(200).json({ success: true, data });
      }
      case "POST": {
        const { data, error } = await supabase.from("backup").insert(body).select().single();
        if (error) throw error;
        return res.status(201).json({ success: true, data });
      }
      case "PUT": {
        const { id } = query;
        const { data, error } = await supabase.from("backup").update(body).eq("id", id).select().single();
        if (error) throw error;
        return res.status(200).json({ success: true, data });
      }
      case "DELETE": {
