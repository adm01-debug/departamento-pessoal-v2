...
export default function SegurosVidaPage() {
  ...
  const { data: sinistros = [], isLoading: loadSin } = useQuery({
    ...
    queryFn: async () => {
      const { data: d, error } = await supabase.from('sinistros_seguro' as any).select('*, colaborador:colaboradores(nome_completo), seguro:seguros_vida(seguradora)').order('created_at', { ascending: false });
      if (error) throw error;
      return d || [];
    },
    enabled: !!empresaAtual?.id,
  ...
  const criarSinistro = useMutation({
    mutationFn: async (d: typeof sinForm) => {
      const { error } = await supabase.from('sinistros_seguro' as any).insert({ seguro_vida_id: d.seguro_vida_id || null, colaborador_id: d.colaborador_id || null, tipo: d.tipo || null, data_sinistro: d.data_sinistro || null, descricao: d.descricao || null, status: 'aberto' });
      if (error) throw error;
    },
