  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: profile }, { data: notificacoes }, { data: pontoHoje }, { data: feriasPendentes }, { data: holerites }, { data: beneficios }, { data: comunicados }, { data: treinamentos }] = await Promise.all([
        ...
        supabase.from('').select('competencia, total_liquido, total_proventos').order('competencia', { ascending: false }).limit(3),
        supabase.from('beneficios').select('nome, tipo, valor, status').eq('status', 'ativo').limit(6),
        supabase.from('').select('id, titulo, tipo, created_at').eq('ativo', true).order('created_at', { ascending: false }).limit(5),
        supabase.from('').select('id, nome, status, data_inicio').in('status', ['pendente', 'em_andamento']).limit(3),
      ]);
