...
export const lcpMonitor = {
  init: (config: MonitorConfig = { enabled: true, sampleRate: 0.1 }) => { console.log('lcpMonitor initialized'); },
  measure: (): number => Math.random() * 100,
  report: () => ({ metric: 'lcpMonitor', value: Math.random() * 1000, timestamp: Date.now() }),
