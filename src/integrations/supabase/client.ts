...
const callBridge = async <T = any>(
  ...
  try {
    ...
    if (!res.ok || json.error) {
      ...
      // RPC_ERROR também é silencioso — a página que chamou a RPC trata o erro
      // localmente (toast próprio, fallback visual, etc.).
      const isMissingObject = /Could not find the (function|table)|schema cache|does not exist|column .* does not exist|RPC_ERROR|Falha na chamada RPC|is not in allowlist/i.test(errorMsg);
      if (!isMissingObject) {
