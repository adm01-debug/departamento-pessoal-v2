export interface SeniorConfig { baseUrl: string; token: string; empresa: string; filial?: string; }
...
class SeniorService {
  private config: SeniorConfig | null = null;
  configurar(config: SeniorConfig): void { this.config = config; }
  private getHeaders(): HeadersInit { if (!this.config) throw new Error("Senior não configurado"); return { "Authorization": `Bearer ${this.config.token}`, "Content-Type": "application/json" }; }
