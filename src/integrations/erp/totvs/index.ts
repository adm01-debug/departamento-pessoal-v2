export interface TotvsConfig { baseUrl: string; token: string; empresa: string; filial?: string; }
...
class TotvsService {
  private config: TotvsConfig | null = null;
  configurar(config: TotvsConfig): void { this.config = config; }
  private getHeaders(): HeadersInit { if (!this.config) throw new Error("Totvs não configurado"); return { "Authorization": `Bearer ${this.config.token}`, "Content-Type": "application/json" }; }
...
export const totvsService = new TotvsService();
export default totvsService;
