export interface SankhyaConfig { baseUrl: string; token: string; empresa: string; filial?: string; }
...
class SankhyaService {
  private config: SankhyaConfig | null = null;
  configurar(config: SankhyaConfig): void { this.config = config; }
  private getHeaders(): HeadersInit { if (!this.config) throw new Error("Sankhya não configurado"); return { "Authorization": `Bearer ${this.config.token}`, "Content-Type": "application/json" }; }
...
export const sankhyaService = new SankhyaService();
export default sankhyaService;
