export interface OracleConfig { baseUrl: string; token: string; empresa: string; filial?: string; }
...
class OracleService {
  ...
  configurar(config: OracleConfig): void { this.config = config; }
  private getHeaders(): HeadersInit { if (!this.config) throw new Error("Oracle não configurado"); return { "Authorization": `Bearer ${this.config.token}`, "Content-Type": "application/json" }; }
